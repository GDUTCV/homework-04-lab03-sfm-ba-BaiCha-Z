...
def create_feature_matches(image_file1: os.path, image_file2: os.path, lowe_ratio: float = 0.6, min_matches: int = 10):
    ...
    # Apply ratio test
    for x, y in matches:
        if x.distance < lowe_ratio * y.distance:
...
def create_scene_graph(image_files: list, min_num_inliers: int = 40):
    ...
    <min_num_inliers> 
    """
    # 遍历所有图像ID组合，进行匹配并添加图像对的边
    for i in range(len(image_ids)):
        id_1 = image_ids[i]  # 获取第一个图像的ID
        for j in range(i + 1, len(image_ids)):
            id_2 = image_ids[j]  # 获取第二个图像的ID
            match_id = '{}_{}'.format(id_1, id_2)
            match_save_file = os.path.join(RANSAC_MATCH_DIR, match_id + '.npy')  # 保存路径

            # 检查匹配文件是否存在
            if os.path.exists(match_save_file):
                inliers = np.load(match_save_file)  # 加载文件
                if len(inliers) > min_num_inliers:
                    graph.add_edge(i, j)  # 在图中添加边
